...
const AboutTronado: React.FC = () => {
  ...
  return (
    <section id="about" className="bg-[#0d0b20] text-white py-20 px-6 md:px-20 overflow-hidden">
      <div className="flex flex-col md:flex-row gap-16">
        ...
        <div className="relative flex-1">
          ...
          {/* Timeline items */}
          <ul className="space-y-16 pl-14">
            {items.map((item, idx) => (
              <li key={idx} className="relative">
                {/* Circular Dot */}
                <div className="absolute -left-9 top-1 w-7 h-7 rounded-full bg-[#0d0b20] border-4 border-[#7450f7] flex items-center justify-center shadow-[0_0_10px_#7450f7]">
                  <div className="w-2.5 h-2.5 rounded-full bg-[#7450f7]" />
                </div>

                {/* Content */}
                <h3 className="text-lg font-bold text-[#b197fc] mb-2">{item.title}</h3>
        ...
        <div className="flex-1 flex justify-center items-start pt-4">
          <div className="p-6 bg-[#19103b] rounded-3xl shadow-lg hover:shadow-purple-500/50 transition-shadow duration-300 cursor-pointer">
            <img
              src="/about_image.svg"
              alt="Tronado Crypto Illustration"
              className="w-[320px] md:w-[400px] transition-transform duration-300 hover:scale-105"
            />
          </div>
        </div>
      </div>
